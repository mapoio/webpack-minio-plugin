...
module.exports = class MinioPlugin {
  constructor(options = {}) {
    ...
    this.uploadOptions = minioUploadOptions
    this.isConnected = false
    this.urlMappings = []
    this.uploadTotal = 0
    this.uploadProgress = 0
    this.basePathTransform = basePathTransform
    basePath = basePath ? addTrailingMinioSep(basePath) : ''
  ...
  apply(compiler) {
    ...
    const isDirectoryUpload = !!this.options.directory,
          hasRequiredUploadOpts = _.every(REQUIRED_MINIO_UP_OPTS, type => this.uploadOptions[type])
  ...
  async handleErrors(error, compilation) {
    compileError(compilation, `MINIOPlugin: ${error}`)
    throw error
  }
  ...
  uploadFile(fileName, file) {
    ...
    const minioParams = _.mapValues(this.uploadOptions, (optionConfig) => {
